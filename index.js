...
app.post('/start-focus', async (req, res) => {
  try {
    ...
    if (!minutes || minutes <= 0) return res.status(400).send('Invalid duration');
...
app.post('/end-focus', async (req, res) => {
  try {
    ...
    const korisnik = await Korisnik.findById(userId);
    if (!korisnik) return res.status(404).send('User not found');
    if (!korisnik.isInFocus || !korisnik.focusStartTime) return res.status(400).send('No active focus session');
